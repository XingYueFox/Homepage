# 星月_Fox的个人主页

这是一个基于 [SRInternet 开发的 Home_Page](https://github.com/SRInternet/Home_Page/tree/master)的二次创作版本。

<img width="2534" height="1599" alt="image" src="https://github.com/user-attachments/assets/2832b2ad-e931-4c49-9b3b-d5dca82a99e8" />

访问[我的网站](https://xingyuefox.pages.dev)

## 🔁 更新内容

此次更新与v26.2.7200相比<br>
1.新增的上半部分标题界面<br>
2.重写导航栏以及交互逻辑<br>
3.重新设计主界面面板<br>
4.优化界面配色方案<br>
5.大版本号由26.2（4700）更新至26.3（8000）


## 📄 相比之下

- ✨ **设计风格**：在大体上与 Home_Page 设计相似
- 🪟 **自适应布局**：重新设计了在移动端上的界面表现效果
- 🎨 **动画效果**：在原有动画之上，我加入了涟漪交互动画
- 📃 **全新的顶部栏**：仿照 Apple 官网为 Homepage 加上全新的可交互顶部栏
- 📈 **活动概览？**：被我换成了个人评估概览（这是计划的一部分
- 📝 **技术栈展示**：更换了配色和描边设计，使页面看上去更加清爽
- 🕒 **最近项目展示**：没那么多项目，被我换成了快速链接
- 🎮 **游戏标签**：拥有流畅的交互特效+浮动卡片效果的游戏板块

## 🛠️ 技术栈

- **后端**：Python 3.6+, Flask
- **前端**：HTML5, Tailwind CSS, JavaScript
- **数据可视化**：Chart.js自行更改
- **第三方集成**：没有
- **部署选项**：Clouldflare Pages，GitHub Pages, Vercel, Netlify等

## ❔ 为什么会有这个
因为好看<br>
你也可以将此当成一个暴力加强版主题<br>

## 🚀 开始
由于改动过大，原Homepage模板不在改动范围（悲
<br>
并且暂不考虑制作模板，如需盗取请自行改动

## 📝 License

[MIT](LICENSE)
